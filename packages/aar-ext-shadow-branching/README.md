...
| `/branch [N]` | Preserve active shadow as `shadow/session-<id>-branch-<K>` and start a fresh branch from `HEAD~N` (or `HEAD` if N is omitted). Reloads session events from the fork point's JSONL so the LLM context matches the new branch. Multiple branches are allowed. Returns `⑂ branch-K preserved as <branch> — now on fresh <branch>`. |
